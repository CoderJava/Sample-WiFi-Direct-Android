...
package com.ysn.examplewifidirect.db;
...
public class DBAdapter {
    ...
    public long addDevice(DeviceDTO device) {
        Log.d("DBAdapter", "addDevice with data device: " + device.toString());
        if (device == null || device.getIp() == null || device.getPort() == 0) {
            Log.d("DBAdapter", "invalid data when addDevice");
            return -1;
        }
        ContentValues contentValues = new ContentValues();
        contentValues.put(DBHelper.COL_DEV_MODEL, device.getDeviceName());
        contentValues.put(DBHelper.COL_DEV_IP, device.getIp());
        contentValues.put(DBHelper.COL_DEV_PLAYER, device.getPlayerName());
        contentValues.put(DBHelper.COL_DEV_PORT, device.getPort());
        contentValues.put(DBHelper.COL_DEV_VERSION, device.getOsVersion());

        if (!deviceExists(device.getIp())) {
            long rowId = db.insert(DBHelper.TABLE_DEVICES, null, contentValues);
            return rowId;
        }
        return -1;
    }
    ...
    public ArrayList<DeviceDTO> getDeviceList() {
        ...
        Cursor cursor = db.query(DBHelper.TABLE_DEVICES, null, null, null, null, null, DBHelper.COL_DEV_PLAYER);
        if (cursor != null) {
    ...
    public DeviceDTO getDevice(String ip) {
        ...
        int modeIndex = cursor.getColumnIndex(DBHelper.COL_DEV_MODEL);
        int ipIndex = cursor.getColumnIndex(DBHelper.COL_DEV_IP);
        int playerNameIndex = cursor.getColumnIndex(DBHelper.COL_DEV_PLAYER);
        int portIndex = cursor.getColumnIndex(DBHelper.COL_DEV_PORT);
        int versionIndex = cursor.getColumnIndex(DBHelper.COL_DEV_VERSION);
        ...
        while (cursor.moveToNext()) {
            device.setDeviceName(cursor.getString(modeIndex));
            device.setIp(cursor.getString(ipIndex));
            device.setPlayerName(cursor.getString(playerNameIndex));
            device.setPort(cursor.getInt(portIndex));
            device.setOsVersion(cursor.getString(versionIndex));
